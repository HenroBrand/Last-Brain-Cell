...
export default function SubmissionView({
  ...
  useEffect(() => {
    ...
    if (timerRemaining === 0 && !hasSubmitted) {
      const finalText = answer.trim() || `Submitting empty brain waves...`;
      onSubmit(finalText);
    }
  }, [timerRemaining, hasSubmitted, answer, onSubmit, isSpectator]);
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-4 flex flex-col items-center">
      ...
      <div className="bg-[#A78BFA] border-3 border-black rounded-lg py-1 px-4 text-xs font-black tracking-widest text-black mb-6 uppercase inline-flex items-center gap-1.5 select-none shadow-[2px_2px_0_#000]">
        <Sparkles className="w-3.5 h-3.5" /> CATEGORY: {challenge.category} <Sparkles className="w-3.5 h-3.5" />
      </div>
        ...
        {isSpectator ? (
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <label htmlFor="answer-input" className="text-xs font-black text-black uppercase tracking-wide text-left">
              WRITE YOUR HILARIOUS SOLUTION:
            </label>
            <textarea
              id="answer-input"
              value={answer}
              onChange={(e) => setAnswer(e.target.value.substring(0, CharacterLimit))}
              placeholder="Write your hilarious solution here... Absurd ideas welcome!"
              rows={4}
            ...
            >
              <Send className="w-5 h-5 shrink-0" /> SUBMIT ANSWER GO!
            </button>
          </form>
        ) : (
          <motion.div
